...
exports.postsRoutes.get("/me", token_middleware_1.tokenMiddleware, async (context) => {
    try {
        ...
        const result = await (0, post_controller_1.GetAllPostsForUser)({ userId, page, limit });
        return context.json(result, 200);
    }
    catch (error) {
        console.error(error);
        if (error === post_types_1.GetPostsError.POSTS_NOT_FOUND) {
...
exports.postsRoutes.delete("/:id", token_middleware_1.tokenMiddleware, async (context) => {
    try {
        ...
        const result = await (0, post_controller_1.deletePost)({ postId, userId });
