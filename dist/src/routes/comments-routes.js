...
const comment_types_1 = require("../controllers/comments/comment-types");
...
exports.commentsRoutes.get("/on/:postId", token_middleware_1.tokenMiddleware, async (c) => {
    try {
        ...
        if (error === comment_types_1.GetCommentsError.POST_NOT_FOUND) {
            return c.json({ error: "Post not found" }, 404);
        }
        if (error === comment_types_1.GetCommentsError.COMMENTS_NOT_FOUND) {
            return c.json({ error: "No comments found on this post" }, 404);
        }
        if (error === comment_types_1.GetCommentsError.PAGE_BEYOND_LIMIT) {
...
exports.commentsRoutes.post("/on/:postId", token_middleware_1.tokenMiddleware, async (c) => {
    try {
        ...
        if (error === comment_types_1.CreateCommentError.POST_NOT_FOUND) {
            return c.json({ error: "Post not found" }, 404);
        }
        if (error === comment_types_1.CreateCommentError.INVALID_INPUT) {
...
exports.commentsRoutes.patch("/:commentId", token_middleware_1.tokenMiddleware, async (c) => {
    try {
        ...
        if (error === comment_types_1.UpdateCommentError.COMMENT_NOT_FOUND) {
            return c.json({ error: "Comment not found" }, 404);
        }
        if (error === comment_types_1.UpdateCommentError.INVALID_INPUT) {
            return c.json({ error: "Comment content is required" }, 400);
        }
        if (error === comment_types_1.UpdateCommentError.NO_CHANGES) {
            return c.json({ error: "No changes detected in comment content" }, 400);
        }
        if (error === comment_types_1.UpdateCommentError.UNAUTHORIZED) {
...
exports.commentsRoutes.delete("/:commentId", token_middleware_1.tokenMiddleware, async (c) => {
    try {
        ...
        if (error === comment_types_1.DeleteCommentError.COMMENT_NOT_FOUND) {
            return c.json({ error: "Comment not found" }, 404);
        }
        if (error === comment_types_1.DeleteCommentError.UNAUTHORIZED) {
